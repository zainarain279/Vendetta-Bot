...
function loadProxies() {
  try {
    ...
    console.log(`Total ${proxyList.length} proxies trong proxies.txt`);
    return proxyList;
  } catch (error) {
...
async function processWallet(wallet, proxyAgent) {
  console.log(`\n🔹 Running wallet: ${wallet.publicKey.substring(0, 15)}...`);
  ...
  if (updatedProfile) {
    ...
    if (updatedProfile.user_leaderboard) {
      console.log(`Total Points: ${updatedProfile.user_leaderboard.totalPoints || 0}`);
      console.log(`Rank: ${updatedProfile.user_leaderboard.rank || 'N/A'}`);
    }
  ...
  if (completedQuests.length > 0) {
    console.log(`✅ Find ${completedQuests.length} mission`);
...
async function main() {
  console.log('Vendetta Auto Task');
  console.log('----------------------------');
  ...
  console.log(`Total ${wallets.length} wallets in .env`);
